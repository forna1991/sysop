...
int checkInputs() {
    if (tflag == 1) {
        if (fflag == 1 || xflag == 1 || cflag == 1) {
            ...
            syslog(LOG_NOTICE, "(EE) -t usato insieme ad altre opzioni!\n");
            return 10;
        } else if (tvalue == NULL) {
            //se non viene immesso niente come parametro del tvalue ritorna 11
            printf("l\'opzione -t non ha un obiettivo!\n");
            syslog(LOG_NOTICE, "(EE) -t usato senza obiettivo!\n");
            return 11;
        } else {
            syslog(LOG_NOTICE, "(II) Lettura archivio\n");
            return 0;
        }
    }
    if (xflag == 1) {
        if (fflag == 0) {
            ...
            syslog(LOG_NOTICE, "(II) Estrazione archivio");
            return 1;
        }
    }
    if (cflag == 1) {
        if (fflag == 0) {
            ...
            syslog(LOG_NOTICE, "(EE) -c passato senza -f!");
            return 31;
        } else if (fvalue == NULL) {
            ...
            syslog(LOG_NOTICE, "(II) Compressione archivio");
            return 2;
        }
    }
    if (fflag == 1) {
        ...
        syslog(LOG_NOTICE, "(EE) -f inserito da solo!");
        return 40;
    } else {
        ...
        syslog(LOG_NOTICE, "(II) nessun input passato stampa dell\'help");
        return 50;
    }
}
...
void work(int optindex, int argc, char** targets) {
    ...
    FILE *file;
    if (code >= 10) {
        syslog(LOG_NOTICE, "(EE) Errore negli input abort! \n");
        exit(0);
    } else if (code == 0) {
        if (file = fopen(tvalue, "r")) {
            ...
            if (cmp == 0) { //se finisce con ".bkp"
                syslog(LOG_NOTICE, "(II) Lettura del file di backup alla posizione "
                        "%s\n", tvalue);
                print(file);
            } else {
                syslog(LOG_NOTICE, "(EE) Il file indicato non e' stato "
                        "riconosciuto come file di backup %s\n", fvalue);
                printf("Il file non sembra essere un file di backup exiting\n");
                exit(0);
            }
        } else {
            syslog(LOG_NOTICE, "(EE) File non trovato alla posizione %s\n", tvalue);
        ...
        if (file = fopen(fvalue, "r")) {
            ...
            if (cmp == 0) { //se finisce con .bkp
                syslog(LOG_NOTICE, "(II) Estrazione del backup alla posizione %s "
                        "nella posizione %s_d\n", fvalue, fvalue);
                extractBkp(file);
            } else {
                syslog(LOG_NOTICE, "(EE) Il file indicato non e' stato "
            ...
            syslog(LOG_NOTICE, "(EE) File di backup non trovato alla posizione "
                    "%s\n", fvalue);
        ...
        if (argc <= optindex) {
            syslog(LOG_NOTICE, "(EE) nessun parametro passato in input!\n");
            ...
            syslog(LOG_NOTICE, "(II) Inizio creazione del backup in %s\n", fvalue);
...
void createBackup(char* bkpPath, char* target) {
    ...
    if (stat(tmp, &s) == 0) { //metodo per verificare se il path tmp e' una cartella
        if (s.st_mode & S_IFREG) { //se e' un file lo aggiunge al bkp
            ...
            char tm[512];
            sprintf(tm, "%s\n", tmp);
            syslog(LOG_NOTICE, "(!!) File trovato %s\n", tmp);
            ...
            syslog(LOG_NOTICE, "(!!) Directory trovata %s\n", tmp);
        ...
        syslog(LOG_NOTICE, "(WW) File non esistente \"%s\"\n", tmp);
    }
}
...
static void recMkdir(const char *dir) {
    ...
    syslog(LOG_NOTICE, "(!!) Creo il path %s\n", dir);
